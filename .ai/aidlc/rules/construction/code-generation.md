...
- [ ] Create explicit steps for unit generation:
  ...
  - Business Logic Unit Testing *(apply `test-driven-development` skill: write failing test first, watch it fail, then implement)*
  - Business Logic Summary
  - API Layer Generation
  - API Layer Unit Testing *(apply `test-driven-development` skill)*
  - API Layer Summary
  - Repository Layer Generation
  - Repository Layer Unit Testing *(apply `test-driven-development` skill)*
  - Repository Layer Summary
  - Frontend Components Generation (if applicable)
  - Frontend Components Unit Testing *(apply `test-driven-development` skill)* (if applicable)
...
> **Skill delegation**: Invoke the `writing-plans` skill to produce this document. AIDLC owns the content (steps, story traceability, exact paths, unit context); `writing-plans` governs format, granularity, and the plan document header. Save location is `aidlc-docs/construction/plans/{unit-name}-code-generation-plan.md` — this overrides the skill's default save path.
...
> **Skill delegation**: Invoke the `subagent-driven-development` skill to execute this plan. AIDLC owns the plan, approval gates, and audit trail; the skill owns execution mechanics (fresh subagent per task, two-stage spec + quality review after each task). If subagents are not available in the current platform, fall back to the `executing-plans` skill.
